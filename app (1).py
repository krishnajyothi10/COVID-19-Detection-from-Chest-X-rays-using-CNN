...
if TF_AVAILABLE:
    ...
    try:
        ...
        st.warning(
            "TensorFlow is available but the model file `best_model.h5` "
            "could not be loaded.\n\n"
            f"Error: {e}\n\n"
    ...
    st.warning(
        ...
        "The app will run in **demo mode** with random predictions only."
    )
...
st.markdown(
    """
This app uses a **Convolutional Neural Network (CNN)** (when available) to classify chest X-ray images as:

- 🦠 **Covid**
- ✅ **Normal**
- 🫁 **Viral Pneumonia**

If TensorFlow or the model is not available, the app runs in **demo mode** with random predictions.
    """
)
...
def make_prediction(image: Image.Image, size: int):
    ...
    if TF_AVAILABLE and model is not None:
        ...
        # Demo / fallback: random probabilities
        probs = np.random.dirichlet(alpha=[2, 2, 2])  # random but sums to 1
...
if uploaded_file is not None:
    ...
    if st.button("🔍 Run Prediction"):
        ...
        mode_text = "Real model" if (TF_AVAILABLE and model is not None) else "Demo (random)"
