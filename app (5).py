...
# Try importing TensorFlow (show a friendly message if missing)
try:
    import tensorflow as tf
except ModuleNotFoundError:
    st.error(
        "❌ TensorFlow is not installed in this environment.\n\n"
        "If you are on Streamlit Cloud, add the line `tensorflow` to your "
        "`requirements.txt` file in the same repo, then redeploy."
    )
    st.stop()
