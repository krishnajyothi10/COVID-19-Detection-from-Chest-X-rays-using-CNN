...
# Class names (used for both real and dummy model)
class_names = ["Covid", "Normal", "Viral Pneumonia"]

# -------------------------------
# Dummy model (used if TF/model not available)
# -------------------------------
class DummyModel:
    def predict(self, x):
        # x is a batch, return random probs for each sample
        probs = np.random.dirichlet(alpha=[2, 2, 2], size=x.shape[0])
        return probs
...
@st.cache_resource
def load_model():
    # If TensorFlow is not available, fall back to dummy
    if not TF_AVAILABLE:
        st.warning(
            "TensorFlow is not available in this environment. "
            "Running in **demo mode** with a dummy model."
        )
        return DummyModel()

    # TensorFlow is available: try to load the real model
    try:
        model = tf.keras.models.load_model("best_model.h5")
        return model
    except Exception as e:
        st.warning(
            "Could not load `best_model.h5`.\n\n"
            f"Reason: {e}\n\n"
            "Running in **demo mode** with a dummy model instead."
        )
        return DummyModel()
...
st.markdown(
    """
This app uses a **Convolutional Neural Network (CNN)** (or a **demo model** if the real model
is not available) to classify chest X-ray images as:
