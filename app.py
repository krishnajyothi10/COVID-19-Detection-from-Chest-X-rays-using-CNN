...
# -------------------------------
# Load model (cached) – only if TF is available
# -------------------------------
@st.cache_resource
def load_model():
    if not TF_AVAILABLE:
    ...
    try:
        ...
        st.warning(
            "TensorFlow is available, but the model file `best_model.h5` could not be loaded.\n\n"
            f"Details: {e}\n\n"
            "The app will run in demo mode with fake predictions."
        )
        return None
...
# Info about mode (real or demo)
if not TF_AVAILABLE or model is None:
    st.warning(
        "⚠️ TensorFlow/model not available in this environment.\n\n"
        "The app is running in **demo mode** using random predictions. "
        "For real predictions, run locally with TensorFlow installed and `best_model.h5` present."
    )
else:
    st.success("✅ TensorFlow model loaded successfully. Running in **real prediction** mode.")

st.markdown(
    """
This app uses a **Convolutional Neural Network (CNN)** (or a demo approximation) to classify chest X-ray images as:
...
def make_prediction(image: Image.Image, size: int):
    """
    If TensorFlow + model are available → real prediction.
    Otherwise → random 'demo' prediction.
    """
    if TF_AVAILABLE and (model is not None):
        ...
        probs = np.random.dirichlet(alpha=[2, 2, 2])
...
st.markdown("---")
st.caption(
    "⚠️ In this deployment, predictions may be **demo only** if TensorFlow/model are unavailable. "
